...
QUESTIONS = [
    # Statistics
    {
        ...
        "answer": "Type I error (false positive) occurs when we reject a true null hypothesis. Type II error (false negative) occurs when we fail to reject a false null hypothesis. In ML terms, Type I is like flagging a legitimate transaction as fraud, while Type II is missing actual fraud. The tradeoff between these errors is controlled by the significance level (alpha) and power (1-beta) of the test.",
        "category": "Statistics",
        "difficulty": "easy",
        "company_tags": "Google|Meta|Amazon",
        "topic_tags": "hypothesis testing|statistical inference",
    },
    {
        "id": "2",
        "question": "What is the Central Limit Theorem and why is it important?",
        "answer": "The Central Limit Theorem states that the sampling distribution of the sample mean approaches a normal distribution as sample size increases, regardless of the population's distribution. This is crucial because it allows us to make inferences about population parameters using normal distribution properties, enables hypothesis testing and confidence intervals, and justifies many statistical methods even when the underlying data isn't normally distributed.",
        "category": "Statistics",
        "difficulty": "easy",
        "company_tags": "Google|Meta|Netflix",
        "topic_tags": "probability|distributions",
    },
    {
        "id": "3",
        "question": "How would you handle multicollinearity in a regression model?",
        "answer": "Multicollinearity can be addressed by: 1) Removing highly correlated features based on VIF (Variance Inflation Factor) > 5-10, 2) Using regularization (Ridge/Lasso) which shrinks correlated coefficients, 3) PCA to create uncorrelated components, 4) Domain knowledge to select the most meaningful feature among correlated ones. The choice depends on whether you need interpretable coefficients (remove features) or just prediction (regularization).",
        "category": "Statistics",
        "difficulty": "medium",
        "company_tags": "Meta|Airbnb|Uber",
        "topic_tags": "regression|feature selection",
    },
    # ML Theory
    {
        "id": "4",
        "question": "Explain the bias-variance tradeoff.",
        "answer": "The bias-variance tradeoff describes the tension between two sources of prediction error. High bias means the model is too simple and underfits (e.g., linear regression on nonlinear data). High variance means the model is too complex and overfits (e.g., unpruned decision tree). Total error = Bias² + Variance + Irreducible noise. We balance this through model selection, regularization, and ensemble methods. Cross-validation helps find the sweet spot.",
        "category": "ML Theory",
        "difficulty": "easy",
        "company_tags": "Google|Amazon|Microsoft",
        "topic_tags": "fundamentals|model selection",
    },
    {
        "id": "5",
        "question": "What's the difference between bagging and boosting?",
        "answer": "Bagging (Bootstrap Aggregating) trains models in parallel on random subsets of data, then averages predictions. It reduces variance (Random Forest). Boosting trains models sequentially, with each model focusing on errors of previous ones. It reduces bias (XGBoost, AdaBoost). Bagging works well when base models overfit; boosting works well when they underfit. Boosting is more prone to overfitting but often achieves higher accuracy with proper tuning.",
        "category": "ML Theory",
        "difficulty": "medium",
        "company_tags": "Google|Meta|Apple",
        "topic_tags": "ensemble|algorithms",
    },
    {
        "id": "6",
        "question": "How does gradient descent work and what are its variants?",
        "answer": "Gradient descent iteratively updates parameters in the direction of steepest descent of the loss function: θ = θ - α∇L(θ). Variants include: Batch GD (uses all data, stable but slow), Stochastic GD (one sample, noisy but fast), Mini-batch GD (compromise). Advanced optimizers like Adam combine momentum (accumulates past gradients) and RMSprop (adaptive learning rates). Choice depends on dataset size, convergence needs, and computational resources.",
        "category": "ML Theory",
        "difficulty": "medium",
        "company_tags": "Google|DeepMind|OpenAI",
        "topic_tags": "optimization|training",
    },
    # Deep Learning
    {
        "id": "7",
        "question": "Explain the vanishing gradient problem and how to address it.",
        "answer": "Vanishing gradients occur when gradients become very small during backpropagation in deep networks, preventing early layers from learning. Causes include sigmoid/tanh activations (derivatives < 1). Solutions: 1) ReLU activation (gradient = 1 for positive inputs), 2) Batch/Layer normalization (stabilizes activations), 3) Residual connections (skip connections allow gradient flow), 4) Proper weight initialization (Xavier/He). Modern architectures like ResNet and Transformers incorporate these solutions.",
        "category": "Deep Learning",
        "difficulty": "medium",
        "company_tags": "Google|Meta|OpenAI",
        "topic_tags": "neural networks|training",
    },
    {
        "id": "8",
        "question": "What is the attention mechanism and why is it important?",
        "answer": "Attention allows models to focus on relevant parts of input when producing output. It computes weighted combinations of values based on query-key similarity: Attention(Q,K,V) = softmax(QK^T/√d)V. Importance: 1) Captures long-range dependencies without sequential processing, 2) Provides interpretability through attention weights, 3) Enables parallelization (vs RNNs). Self-attention (Transformers) revolutionized NLP and is now used in vision (ViT) and other domains.",
        "category": "Deep Learning",
        "difficulty": "medium",
        "company_tags": "Google|OpenAI|Anthropic",
        "topic_tags": "transformers|architectures",
    },
    {
        "id": "9",
        "question": "How would you handle class imbalance in deep learning?",
        "answer": "Strategies include: 1) Data-level: oversampling minority (SMOTE), undersampling majority, data augmentation, 2) Algorithm-level: class weights in loss function, focal loss (down-weights easy examples), threshold adjustment, 3) Ensemble: combine models trained on balanced subsets. For neural networks specifically: stratified batching, two-phase training (pretrain on balanced, fine-tune on original). Evaluation should use precision-recall curves and F1 rather than accuracy.",
        "category": "Deep Learning",
        "difficulty": "hard",
        "company_tags": "Amazon|PayPal|Stripe",
        "topic_tags": "imbalanced data|training",
    },
    # NLP
    {
        "id": "10",
        "question": "Explain word embeddings and their evolution.",
        "answer": "Word embeddings map words to dense vectors capturing semantic meaning. Evolution: 1) One-hot encoding (sparse, no semantics), 2) Word2Vec/GloVe (static embeddings from co-occurrence), 3) ELMo (contextualized via bidirectional LSTM), 4) BERT/GPT (contextualized via Transformers). Key insight: words with similar contexts have similar vectors. Modern embeddings are contextual (same word gets different vectors based on context) and can be fine-tuned for downstream tasks.",
        "category": "NLP",
        "difficulty": "medium",
        "company_tags": "Google|OpenAI|Meta",
        "topic_tags": "embeddings|representations",
    },
    {
        "id": "11",
        "question": "What are the key differences between BERT and GPT?",
        "answer": "BERT (Bidirectional Encoder Representations from Transformers) uses masked language modeling and sees full context bidirectionally. Best for understanding tasks (classification, NER, QA). GPT (Generative Pre-trained Transformer) uses autoregressive language modeling, predicting next token left-to-right. Best for generation tasks. BERT is encoder-only, GPT is decoder-only. For tasks needing both understanding and generation, encoder-decoder (T5) or large autoregressive models (GPT-4) with in-context learning work well.",
        "category": "NLP",
        "difficulty": "medium",
        "company_tags": "Google|OpenAI|Microsoft",
        "topic_tags": "transformers|language models",
    },
    # System Design
    {
        "id": "12",
        "question": "Design a recommendation system for an e-commerce platform.",
        "answer": "Components: 1) Data collection: user behavior (clicks, purchases, time), item features, context (time, device), 2) Candidate generation: collaborative filtering (user-item matrix factorization), content-based (item similarity), 3) Ranking: ML model combining features (user, item, context) to predict engagement, 4) Serving: precompute for cold start, real-time for logged-in users, 5) Feedback loop: A/B testing, handling cold start (popular items, explore/exploit). Key tradeoffs: latency vs personalization, diversity vs relevance, short vs long-term engagement.",
        "category": "System Design",
        "difficulty": "hard",
        "company_tags": "Amazon|Netflix|Spotify",
        "topic_tags": "recommendations|architecture",
    },
    {
        "id": "13",
        "question": "How would you design an ML pipeline for real-time fraud detection?",
        "answer": "Architecture: 1) Data ingestion: Kafka for streaming transactions, 2) Feature engineering: real-time features (velocity, device fingerprint) + batch features (historical patterns), 3) Model: ensemble of rules + ML (isolation forest, XGBoost) for sub-100ms latency, 4) Serving: feature store for consistency, model versioning, 5) Feedback: human review loop, delayed labels, continuous retraining. Key considerations: class imbalance, adversarial adaptation, explainability for disputes, cost of false positives/negatives.",
        "category": "System Design",
        "difficulty": "hard",
        "company_tags": "PayPal|Stripe|Square",
...
def get_random_question(categories: list, difficulties: list, state: dict) -> dict:
    ...
    if difficulties and "All" not in difficulties:
        filtered = filtered[
            filtered["difficulty"].isin([d.lower() for d in difficulties])
        ]
...
def start_quiz(
    categories: list, difficulties: list, state: dict
...
def browse_questions(category: str, difficulty: str, search: str) -> str:
    ...
    if search:
        mask = filtered["question"].str.contains(search, case=False) | filtered[
            "answer"
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Quiz Mode"):
            ...
            status_text = gr.Textbox(
                label="Status", value="Click 'Next Question' to start"
            )
