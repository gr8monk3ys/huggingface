"""The arXiv paper corpus, shared by every Space that reads it.
...
Papers come from the published Hub dataset when it is reachable, and from
FALLBACK_PAPERS when it is not. The fallback is a degraded mode users are not
told about directly, so its contents are a product surface, not a fixture.
"""
