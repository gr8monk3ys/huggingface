"""Prompt enhancement logic, independent of Gradio and of the Inference API.

The entry point is :func:`enhance_prompt`, which owns the sequence: validate,
resolve the prompt-type config, apply the enhancement level, call the model, and
return an :class:`Enhancement`. See
...
# Appended to the type's system prompt to set how far the rewrite may go.
LEVEL_INSTRUCTIONS = {
...
DEFAULT_TYPE = "Text/Chat"
MAX_TOKENS = 800
TOP_P = 0.9


class InputError(ValueError):
    """The caller supplied no prompt to enhance."""
...
class Enhancement:
    """The result of enhancing one prompt."""

    enhanced: str
    tips: str


def tips_for(prompt_type: str) -> str:
...
def example_for(prompt_type: str) -> tuple[str, str]:
...
def enhance_prompt(
    ...
    *,
    ...
    """Rewrite *basic_prompt* as a stronger prompt of the requested type.

    An unrecognised *prompt_type* or *enhancement_level* falls back to a default
    rather than failing: both come from dropdowns, so a bad value means a UI
    change, not bad input.

    Raises:
        InputError: *basic_prompt* is empty or whitespace.
        Exception: whatever *chat_fn* raises.
    """
    if not basic_prompt.strip():
        raise InputError("Please enter a prompt to enhance.")

    config = PROMPT_TYPES.get(prompt_type, PROMPT_TYPES[DEFAULT_TYPE])
    ...
    return Enhancement(enhanced=enhanced.strip(), tips=tips_for(prompt_type))
